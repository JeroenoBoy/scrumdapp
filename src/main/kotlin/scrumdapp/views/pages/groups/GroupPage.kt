...
fun FlowContent.userEditWidget(users: List<User>, userGroups: List<UserGroup>) {
    form(method=FormMethod.put, classes="vertical g-md flex-1") {
        table(classes="checkin-table") {
            ...
            tbody {
                for (user in users) {
                    val userPermission = userGroups.find { it.userId == user.id }?.permission ?: 69
                    tr {
                        ...
                        td(classes="pl-md") {
                            select(classes = "input select-role w-full text-ellipse") { name="role"
                                option(classes="red",) {
                                    value="-2";
                                    if (userPermission > -2) attributes["disabled"] = ""
                                    if (userPermission == -2) attributes["selected"] = ""
                                    +"Scrumdad+"}
                                option(classes="yellow") {
                                    value="-1";
                                    if (userPermission > -1) attributes["disabled"] = ""
                                    if (userPermission == -1) attributes["selected"] = ""
                                    +"Scrumdad"}
                                option(classes="blue") {
                                    value="0";
                                    if (userPermission > 0) attributes["disabled"] = ""
                                    if (userPermission == 0) attributes["selected"] = ""
                                    +"User management"}
                                ...
                                option(classes="aqua") {
                                    value="69";
                                    if (userPermission == 69) attributes["selected"] = ""
                                    +"Lid"}
                            }
                        }
                    }
